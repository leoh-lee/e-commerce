...
class KafkaSchedulerTest {
    ...
    @BeforeEach
    void setUp() {
        KafkaTopicsProperties.OrderTopics orderTopics = getOrderTopics();
        when(kafkaTopicsProperties.getOrder()).thenReturn(orderTopics);
        orderCreateTopic = orderTopics.getCreated();
    }
    ...
    void sendOutboxMessage_happyPath() throws Exception {
        ...
        when(outboxService.getByTopicContainingAndStatusNotSuccess(orderCreateTopic)).thenReturn(outboxes);
        ...
        verify(orderEventProducer).publishOrderEvent(eq(orderCreateTopic), any(OrderCreatedEvent.class));
    ...
    void sendOutboxMessage_whenPublishThrowsException_updatesStatusToFailed() throws Exception {
        ...
        when(outboxService.getByTopicContainingAndStatusNotSuccess(orderCreateTopic)).thenReturn(outboxes);
        doThrow(RuntimeException.class).when(orderEventProducer).publishOrderEvent(eq(orderCreateTopic), any(OrderCreatedEvent.class));
    ...
    void sendOutboxMessage_whenJsonProcessingException_throwsRuntimeException() {
        // given
        Outbox outbox = new Outbox(1L, "1", orderCreateTopic, OutboxStatus.PENDING, "invalid payload", LocalDateTime.now(), 0);
    ...
    private Outbox createTestOutbox(int retry) throws JsonProcessingException {
        ...
        return new Outbox(1L, "1", orderCreateTopic, OutboxStatus.PENDING, payload, LocalDateTime.now(), retry);
    }
